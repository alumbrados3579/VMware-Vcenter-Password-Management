...
# Version 1.2 - DoD Compliant Edition with Chunked Module Support
...
# Module chunks configuration
$script:ModuleChunks = @(
    "modules-chunk-01.zip",
    "modules-chunk-02.zip", 
    "modules-chunk-03.zip",
    "modules-chunk-04.zip"
)
...
function Test-Prerequisites {
    ...
    try {
        ...
        if ($freeSpaceGB -gt 1.0) {
            ...
            $results.Recommendations += "Free up at least 1GB of disk space for module extraction"
...
function Extract-ModuleChunks {
    param(
        [string]$DownloadDirectory
    )
    
    Write-StartupLog "Starting module chunk extraction" "INFO" -ToConsole
    Write-Host "Extracting PowerCLI module chunks..." -ForegroundColor Cyan
    ...
    foreach ($chunkFile in $script:ModuleChunks) {
        $chunkPath = Join-Path $DownloadDirectory $chunkFile
        
        if (Test-Path $chunkPath) {
            Write-Host "Extracting: $chunkFile" -ForegroundColor Cyan
            Write-StartupLog "Extracting chunk: $chunkFile" "INFO"
            
            try {
                # Use .NET to extract zip file
                Add-Type -AssemblyName System.IO.Compression.FileSystem
                [System.IO.Compression.ZipFile]::ExtractToDirectory($chunkPath, $DownloadDirectory)
                
                Write-Host "✅ Successfully extracted: $chunkFile" -ForegroundColor Green
                Write-StartupLog "Successfully extracted chunk: $chunkFile" "SUCCESS"
                
                # Add to cleanup list
                $chunksToCleanup += $chunkPath
                
            } catch {
                Write-Host "❌ Failed to extract: $chunkFile - $($_.Exception.Message)" -ForegroundColor Red
                Write-StartupLog "Failed to extract chunk $chunkFile`: $($_.Exception.Message)" "ERROR"
                $extractedSuccessfully = $false
            }
        } else {
            Write-Host "⚠️ Chunk file not found: $chunkFile" -ForegroundColor Yellow
            Write-StartupLog "Chunk file not found: $chunkFile" "WARN"
        }
    }
    ...
    # Verify extraction
    $extractedModules = Get-ChildItem -Path $modulesDir -Directory -ErrorAction SilentlyContinue | Where-Object { $_.Name -like "VMware.*" }
    
    if ($extractedModules.Count -gt 0) {
        Write-Host "✅ PowerCLI modules extracted: $($extractedModules.Count) modules available" -ForegroundColor Green
        Write-StartupLog "PowerCLI modules extracted: $($extractedModules.Count) modules" "SUCCESS"
        return $true
    } else {
        Write-Host "❌ No PowerCLI modules found after extraction" -ForegroundColor Red
        Write-StartupLog "No PowerCLI modules found after extraction" "ERROR"
        return $false
    }
}
...
function Show-DownloadOptions {
    ...
    if ($script:HasGUI) {
        ...
        $lblFullDesc = New-Object System.Windows.Forms.Label -Property @{
            Text = "Downloads all scripts, documentation, tools, and PowerCLI modules. Includes chunked PowerCLI modules that will be automatically extracted and cleaned up. Perfect for first-time setup."
        ...
        $lblProgress = New-Object System.Windows.Forms.Label -Property @{
            Text = "Download will create a complete local directory structure with all necessary files and documentation from GitHub. PowerCLI modules will be automatically extracted and zip files cleaned up."
...
function Start-Download {
    ...
    switch ($DownloadType) {
        "Full" {
            ...
            # Add module chunks for full download
            foreach ($chunkFile in $script:ModuleChunks) {
                $filesToDownload += @{ Name = "PowerCLI module chunk: $chunkFile"; GitHubPath = $chunkFile; LocalPath = $chunkFile; Required = $true }
            }
        }
        "Scripts" {
            $filesToDownload = @(
    ...
    # Extract PowerCLI modules if this is a full download
    if ($DownloadType -eq "Full") {
        if ($statusLabel) {
            $statusLabel.Text = "Extracting PowerCLI modules..."
            $statusLabel.Refresh()
        }
        
        $extractionSuccess = Extract-ModuleChunks -DownloadDirectory $DownloadDirectory
        
        if ($extractionSuccess) {
            Write-Host "✅ PowerCLI modules extracted and cleaned up successfully" -ForegroundColor Green
            Write-StartupLog "PowerCLI modules extracted and cleaned up successfully" "SUCCESS"
        } else {
            Write-Host "⚠️ PowerCLI module extraction had issues" -ForegroundColor Yellow
            Write-StartupLog "PowerCLI module extraction had issues" "WARN"
        }
    }
...
"@
    ...
    if ($DownloadType -eq "Full") {
        Write-Host "PowerCLI Modules: Extracted and cleaned up automatically" -ForegroundColor Green
    } else {
    ...
    if ($script:HasGUI) {
        $completionMessage = "Download completed successfully!`n`nDownload Directory: $DownloadDirectory`nFiles Downloaded: $successCount"
        
        if ($DownloadType -eq "Full") {
            $completionMessage += "`nPowerCLI modules extracted and zip files cleaned up automatically"
        }
        
        $completionMessage += "`n`nNext steps:`n1. Edit hosts.txt with your ESXi hosts`n2. Run the main script to begin`n`nWould you like to open the download directory now?"
...
function Start-StartupScript {
    ...
    Write-Host "DoD Compliant Edition - Automated Installation with Chunked Module Support" -ForegroundColor Cyan
    Write-Host ""
